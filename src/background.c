...
// Add coordinates to list of exceptions so that the coordinate isn't used for
// anything else
...
bool emptySquare(int *coord) {
  for (int i = 0; i < total_exceptions; ++i) {
    if (coord[0] == exceptions[i][0] && coord[1] == exceptions[i][1])
      return false;
  }
  return true;
}
  ...
  for (int y = 0; y < HEIGHT; y++) {
    ...
    for (int x = 0; x < WIDTH; x++) {
      if (y == 0 || y == HEIGHT - 1 || x == 0 || x == WIDTH - 1) {
        ...
        // Insert marker
        if (!flag)
          flag = insertObject(marker_coords, total_markers, x, y, 3);
      }
    }
  }
